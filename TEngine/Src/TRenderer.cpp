...
bool TRenderer::InitializeSwapChain()
{
	...
	hResult = D3D11CreateDeviceAndSwapChain(
		...
		&m_SwapChain.GetSwapChainDesc(
			gWindowHandle,
			gContext->gEngineConfigManager->gWindowConfig->GetWindowWidth(),
			gContext->gEngineConfigManager->gWindowConfig->GetWindowHeight()
		),
	...
	// 백버퍼 세팅
	hResult = m_SwapChain.GetSwapChain()->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)(m_SwapChain.GetBackBuffer().GetAddressOf()));
	if (FAILED(hResult))
	{
...
bool TRenderer::InitializeDepthStencil()
{
	...
	// 뎁스 스탠실 버퍼 생성
	hResult = gDevice->CreateTexture2D(
		&m_DepthStencil.GetDepthStencilDesc(
			gContext->gEngineConfigManager->gWindowConfig->GetWindowWidth(),
			gContext->gEngineConfigManager->gWindowConfig->GetWindowHeight()
		),
		NULL,
		m_DepthStencil.GetDepthStencilBuffer().GetAddressOf()
	);
...
{
	gDeviceContext->RSSetViewports(1, &m_ViewPort.GetViewPort(gContext->gEngineConfigManager->gWindowConfig->GetWindowWidth(), gContext->gEngineConfigManager->gWindowConfig->GetWindowHeight()));
...
{
	TWString shaderfolder = L"Shaders\\";
...
bool TRenderer::InitializeScene()
{
	...
	if (mCamera == nullptr)
	{
		...
		mCamera->SetProjectionValues(
			90.0f,
			static_cast<float>(gContext->gEngineConfigManager->gWindowConfig->GetWindowWidth()) / static_cast<float>(gContext->gEngineConfigManager->gWindowConfig->GetWindowHeight()),
			0.1f,
			1000.0f
		);
	}
