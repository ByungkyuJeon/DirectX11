...
bool TPhysics::Initialize()
{
	return true;
}

void TPhysics::registerPhysicsObject(std::shared_ptr<PhysicsObject> InPhysicsObject)
{
	// 환경 물리 연산 대상에 추가
	this->mPhysicsObjects.emplace_back(InPhysicsObject);

	// 충돌 검사 연산 대상에 추가
	if (!InPhysicsObject->getGameObject()->getGameObjectSetting().CollisionDetectionEnabled())
	{
		return;
	}

	if (InPhysicsObject->getGameObject()->getCollider() == nullptr)
	{
		return;
	}

	this->mCollisionObjects.emplace_back(InPhysicsObject);
}

void TPhysics::registerPhysicsObject(std::shared_ptr<GameObject> InGameObject)
{
	std::shared_ptr<PhysicsObject> physicsObject = std::make_shared<PhysicsObject>(InGameObject);
...
size_t TPhysics::getObjectSize() const
{
	return this->mPhysicsObjects.size();
}

std::shared_ptr<PhysicsObject> TPhysics::getObject(size_t idx) const
{
	return this->mPhysicsObjects[idx];
}

void TPhysics::update(float delta)
{
	if (delta > 100) { return; }
