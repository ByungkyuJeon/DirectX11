...
{
	m_SeclectedAdapterDataIndex = 0;

	// 비디오 메모리가 가장 큰 어뎁터 선택
	for (int adapterIdx = 1; adapterIdx < m_Adapters.size(); adapterIdx++)
	{
		if (m_Adapters[adapterIdx].GetAdapterDescription().DedicatedVideoMemory > m_Adapters[m_SeclectedAdapterDataIndex].GetAdapterDescription().DedicatedVideoMemory)
		{
